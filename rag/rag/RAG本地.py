...
def generate(query, retrieved_docs):
    ...
    messages = [
        {
            "role": "system",
            "content": "你是一个知识丰富的 AI 助手。你必须严格按照以下格式回答：1. 先给出答案 2. 空一行 3. 最后一行必须是'置信度：'后跟一个0.00到1.00之间的两位小数",
        },
        {"role": "user", "content": f"以下是相关知识库内容：\n{retrieved_docs[0]}"},
        {
            "role": "user",
            "content": f"问题：{query}\n\n注意：你必须在回答的最后一行给出置信度，由你来判断回答的准确度，数值在0.00到0.99，置信度不允许一直是一个数，要有随机的感觉，且要根据你的知识库来判断是多少并输出，太离谱的可以给到0.5以下，格式为：置信度：0.XX",
        },
    ]
...
if __name__ == "__main__":
    ...
    sample_docs = []
    knowledge_files = [
        r"D:\DUT\freshterm\science_creation\信息安全竞赛\web\rag\baidu_knowledge.txt",
    ]
    ...
    while True:
    #     with websockets.connect("ws://10.242.11.43:8765/rag") as websocket:
    #         # 接收问题
